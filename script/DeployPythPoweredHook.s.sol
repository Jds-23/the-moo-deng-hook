...
import {PythPoweredTheHook} from "src/PythPoweredTheHook.sol";
...
contract DeployHook is Script {
    ...
    function setUp() public {
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER,
            flags,
            type(PythPoweredTheHook).creationCode,
            abi.encode(address(manager))
        );

        PythPoweredTheHook hook = new PythPoweredTheHook{salt: salt}(
            manager,
            UnichainSepoliaDeployments.pythContract
        );

        hook.setPriceFeedId(UnichainSepoliaDeployments.priceFeedId);
